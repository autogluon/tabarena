...
- **In-context models need a size guard, measured rather than guessed.** A TabPFN-style model keeps the training table in GPU memory at predict time; above some size it either raises out-of-memory (TabFM) or, worse, survives on a smaller-batch fallback and crawls for hours until AutoGluon's fold-time projection kills the fit (LimiX-2). Before writing a guard, run the model on tables that vary rows and columns separately: on a 96 GB card LimiX-2 passed 38k x 318 in 4 h but not 53.6k x 243 or 7.9k x 1652 (the same 13M cells), so a rows x columns cell budget is the wrong proxy there. Prefer the library's own knobs (`max_num_rows`, `test_batch_size`, a native feature budget) and engage a wrapper-side cap (stratified row sample, univariate column selection; helpers in `tabarena/utils/wrapper_utils.py`, `models/limix_2/model.py` is the reference) only above a shape that fails, so smaller tables stay untouched. Call the cap an intermediate placeholder in its docstring and name the library-side fix to ask the authors for.
