...
class MarkovBlanketFS:
    ...
    def fit_transform(self, X: pd.DataFrame, y: pd.Series, model, n_max_features, **kwargs) -> pd.DataFrame:
        ...
        kept = self.mb_expected_cross_entropy(
            X, y, n_max_features, k=5, n_eliminate=n_eliminate, use_fi_in_delta=True, **kwargs
        )
        X_selected = X[kept]
    ...
    def mb_expected_cross_entropy(
        ...
        k: int,
        n_eliminate: int,
        use_fi_in_delta: bool = True,
        cv_splits: int = 5,
        random_state: int = 0,
        **kwargs,
    ) -> list[str]:
        r"""Markov-blanket-style elimination.
