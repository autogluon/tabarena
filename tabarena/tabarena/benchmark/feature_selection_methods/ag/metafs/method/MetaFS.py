...
from tabarena.benchmark.feature_selection_methods.ag.metafs.method.Add_Pandas_Metafeatures import (
    add_pandas_metadata_selection_columns,
)
...
class MetaFS:
    ...
    def fit_transform(self, X: pd.DataFrame, y: pd.Series, model, n_max_features, **kwargs) -> pd.DataFrame:
        ...
        result_matrix = pd.read_parquet(
            "../../tabarena/benchmark/feature_selection_methods/ag/metafs/method/Pandas_Matrix_Complete.parquet"
        )
    ...
    def _extract_metadata(y: pd.Series) -> dict:
        n_classes = y.nunique()
        task_type = "Supervised Classification" if n_classes < 200 else "Supervised Regression"
        return {"task_id": None, "task_type": task_type, "number_of_classes": "N/A"}
