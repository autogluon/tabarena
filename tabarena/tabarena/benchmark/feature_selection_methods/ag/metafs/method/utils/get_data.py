...
def get_all_amlb_dataset_ids():
    ...
    # )
    return [146818, 146820, 168350, 168911, 190137, 190411, 359955, 359956, 359979]


def get_openml_dataset_split_and_metadata(
    openml_task_id: int,
...
def get_openml_dataset_split(openml_task_id: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    task = openml.tasks.get_task(
        openml_task_id,
        download_splits=True,
        download_data=True,
...
def get_openml_dataset_and_metadata(openml_task_id: int) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    task = openml.tasks.get_task(
        openml_task_id,
        download_splits=True,
        download_data=True,
...
def get_openml_dataset(openml_task_id: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    task = openml.tasks.get_task(
        openml_task_id,
        download_splits=True,
        download_data=True,
...
def split_data(data, target_label, seed) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    y = data[target_label]
...
def get_dataset_split(dataset_id: int, seed) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    if dataset_id == 1:
