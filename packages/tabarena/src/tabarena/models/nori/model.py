...
class NoriModel(AbstractTorchModel):
    ...
    def _fit(
        ...
        **kwargs,
        ...
        self.model.fit(X, y)
        # NoriRegressor builds its predictor, and with it reads the checkpoint, at the first predict.
        # Building it here keeps that load inside the fit (where the network is shared) and lets the
        # timed predict start with a ready predictor. Library ask: a public way to build it eagerly.
