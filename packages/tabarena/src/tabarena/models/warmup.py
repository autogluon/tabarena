...
and pre-loading shared checkpoint weights into the process-wide weights registry
(:mod:`tabarena.models._weights`) under exactly the key the fit derives. It must never touch the
...
Measured as shipped. Three entrants cannot follow the shared-weights part of the contract and are
measured as their libraries ship: Mitra v1 (the TabArena ``mitra`` entry uses AutoGluon's
``MitraModel`` directly), the AutoGluon system's own foundation-model children (resolved through
``ag_model_registry``), and TabPFN-3-API (no local weights). Time comparisons between them and the
TabArena wrappers must be read with that in mind.
...
class WarmupReport:
    ...
    Attributes:
        ...
        weights_preloaded: ``WeightsEntry.to_metadata()`` of every shared-weights key the warm-up
            primed or found present.
...
def warmup_shared_weights(
    ...
    *,
    ...
    """
    name = f"weights:{model_cls.__name__}"
    key_fn = getattr(model_cls, "shared_weights_key", None)
    loader_fn = getattr(model_cls, "_load_shared_weights", None)
    if key_fn is None or loader_fn is None:
    ...
    if problem_type is None:
        ...
        return
    try:
        from tabarena.models._weights import prime

        device = device_for_num_gpus(
            resolve_warmup_num_gpus(hyperparameters, num_gpus),
            default_num_gpus=getattr(model_cls, "default_num_gpus", 0) or 0,
        )
        key = key_fn(problem_type=problem_type, hyperparameters=strip_ag_args(hyperparameters), device=device)
        if key is None:
            report.step(f"{name}:skipped")
            return
...
def warmup_dummy_fit(
    ...
    *,
    ...
    ``warmup_dummy_fit_hyperparameters``; checkpoint-relevant keys must not be overridden there. The
    ...
    try:
        ...
        with rng_guard(cuda=resolved_gpus > 0 or bool(cuda_initialized())):
            ...
            hps.update(getattr(model_cls, "warmup_dummy_fit_hyperparameters", None) or {})
            model = model_cls(
                path=tmp_dir,
...
def warmup_model_cls(
    ...
    *,
    ...
    5. Shared weights via :func:`warmup_shared_weights` when the class declares the hooks.
    6. The dummy fit via :func:`warmup_dummy_fit`, unless ``dummy_fit`` is ``False`` (the exec
       model's ``warmup_dummy_fit``) or the class opts out.
    ...
    warmup_shared_weights(
        model_cls, problem_type=problem_type, num_gpus=num_gpus, hyperparameters=hyperparameters, report=report
    )
