...
class ILTMModel(AbstractTorchModel):
    ...
    The wrapper does not use the shared-weights registry (``SharedWeightsModelMixin``): the library
    ...
    Injecting a registry module would mean overriding those version-dependent hooks and importing
    ``iltm`` before the first fit, which is the TF32 question described at :attr:`warmup_modules`.
    The checkpoint config (preprocessing, tree embedding, retrieval, bottleneck) is keyed off the
    ...
    """Off for the same reason as :attr:`warmup_modules`: a dummy fit imports ``iltm`` inside the
    untimed warm-up, so the first real fold would no longer run with the import-time TF32 flag. Once
    the TF32 policy is explicit in ``_fit`` this can be switched on; the dummy fit then also fills
    ...
    """Cheapness knobs for the warm-up dummy fit (one ensemble member, one fine-tuning epoch, a tiny
    tree embedding); none of them selects or shapes the checkpoint."""
