...
# NOTE (tabarena vendor): sklearn >= 1.6 removed the ``BaseEstimator._validate_data`` method in
# favor of the module-level ``sklearn.utils.validation.validate_data(estimator, ...)``. Upstream's
# transformers call ``_validate_data(self,...)`` directly, which raises ``AttributeError`` on
# sklearn >= 1.6. This shim dispatches to the function when present, falling back to the method on
# old sklearn; every ``_validate_data(self,...)`` call below was rewritten to ``_validate_data(self,
# ...)``.
try:
...
def _validate_data(estimator, *args, **kwargs):
    if _validate_data_func is not None:
        return _validate_data_func(estimator, *args, **kwargs)
