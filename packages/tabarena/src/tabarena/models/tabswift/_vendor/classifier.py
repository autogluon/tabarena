...
class TabSwiftClassifier(ClassifierMixin, BaseEstimator):
    ...
    def _load_model(self):
        ...
        # NOTE (tabarena vendor): upstream points the auto-download at the private/broken
        # repo "pretrain-models/tabswift" (filenames "tabswift-classifier.ckpt" /
        # "tabswift-regressor.ckpt"). The public checkpoint is a single "swift.ckpt" hosted
        # at "LAMDA-Tabular/TabSwift" (used for both the classifier and the regressor), so we
        # retarget the default here. In practice the TabArena wrapper always passes an explicit
    ...
    def predict_proba(self, X):
        ...
        # fired on every ``predict_proba`` call.
        return avg
