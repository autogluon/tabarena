...
from tabarena.models.tabldm._vendor._model.tabldm import TabLDM
...
class TabLDMBaseEstimator(BaseEstimator):
    """Base class for TabLDM scikit-learn estimators.

    Provides shared functionality for the internal classifier and regressor
    estimators used by the public MoE1 loaders:
    ...
    :class:`tabldm.TabLDMClassifier` or
    :class:`tabldm.TabLDMRegressor` instead.
    """
    ...
    def __setstate__(self, state):
        ...
        if model_state_dict is not None and hasattr(self, "model_config_"):
            # Model weights were saved alongside the estimator (self-contained).
            # Reconstruct from the stored config + state dict; if the estimator
            # overrides the architecture after construction (e.g. AttnRes/MoE
            # variants replace col_embedder), a plain TabLDM(**config) rebuild will
            # reject architecture-specific keys or mismatch the state dict. In
            # that case fall back to _load_model(), which re-runs the estimator's
            # full construction path from the checkpoint.
            try:
                self.model_ = TabLDM(**self.model_config_)
    ...
    def save(
        ...
        save_model_weights : bool, default=False
            If True, include the pretrained TabLDM model weights in the file,
    ...
    @classmethod
    def load(cls, path: str | Path, device: Optional[str | torch.device] = None) -> TabLDMBaseEstimator:
