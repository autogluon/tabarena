...
TabArena reads and writes six independent caches. Historically each was process-global
...
_LOCATION_FIELDS = ("openml", "huggingface", "tabpfn", "data_foundry", "tabarena", "results")
...
class CacheConfig:
    ...
    The six caches and their roles:
    ...
    huggingface:
        ...
        do not live here either; see ``tabpfn``.
    tabpfn:
        The TabPFN checkpoint directory: the ``tabpfn`` package downloads its ``.ckpt`` files
        into its own cache, not into ``HF_HOME``. This is the shared source of truth the
        head-node prefetch fills and every worker reads. Set via the ``TABPFN_MODEL_CACHE_DIR``
        environment variable (read when ``tabpfn.settings`` is imported); when that module is
        already imported, :meth:`apply` also repoints ``settings.tabpfn.model_cache_dir`` in place.
        Library default ``~/.cache/tabpfn``. Only the process :meth:`apply` runs in (and children
        started afterwards) see the variable; the tabpfn wrappers fit their folds sequentially in
        that process.
    data_foundry:
    ...
    def from_root(cls, root: str | Path, **overrides) -> CacheConfig:
        ...
        ``/scratch/me/huggingface``, ``/scratch/me/tabpfn``, ``/scratch/me/data_foundry``,
        ``/scratch/me/tabarena`` and ``/scratch/me/results``. Handy when one large/shared disk
        should hold everything. Any field can be pinned via a keyword override (e.g.
        ``from_root(root, results=None)`` to keep run outputs in a throwaway temp dir, or
        ``from_root(root, scope_openml=True)`` to set a policy flag). ``<root>/tabpfn`` starts
        empty: copy ``~/.cache/tabpfn/*.ckpt`` there, let the prefetch fill it, or pass
        ``tabpfn=None`` to keep TabPFN's default cache.
        """
        root = Path(root)
    ...
    def apply(self, *, openml: bool = True) -> None:
        ...
        ``tabpfn`` via the ``TABPFN_MODEL_CACHE_DIR`` environment variable (see
        :meth:`_apply_tabpfn`), and ``data_foundry`` via the ``DATA_FOUNDRY_CACHE`` environment
        ...
        if self.tabpfn is not None:
            self._apply_tabpfn(Path(self.tabpfn).expanduser())
    ...
    def scoped_openml(self) -> Iterator[None]:
        ...
        * **tabpfn** is frozen into ``tabpfn.settings`` at import time like HuggingFace, so it is
    ...
    @staticmethod
    def _apply_tabpfn(path: Path) -> None:
        """Point the TabPFN checkpoint cache at ``path`` via ``TABPFN_MODEL_CACHE_DIR``.

        ``tabpfn.settings`` reads the variable once, when it is imported, so setting it before the
        first tabpfn import is sufficient. If the module is *already* imported, its
        ``settings.tabpfn.model_cache_dir`` is repointed in place as a ``Path`` (the loader joins
        checkpoint names onto it). The module is never force-imported.
        """
        os.environ["TABPFN_MODEL_CACHE_DIR"] = str(path)

        settings_module = sys.modules.get("tabpfn.settings")
        if settings_module is None:
            return
        tabpfn_settings = getattr(getattr(settings_module, "settings", None), "tabpfn", None)
        if tabpfn_settings is not None:
            tabpfn_settings.model_cache_dir = path
