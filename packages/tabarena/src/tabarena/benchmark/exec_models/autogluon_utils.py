...
validation splitting to a task's structure (tiny-data fold counts, group-wise / time-based
splits), keyed off a :class:`~tabarena.benchmark.task.metadata.ValidationMetadata` (the
task-derived config) rather than a mixin on the wrapper. Its two entry points are
