...
class AbstractExecModel:
    ...
    _split_seed: Literal["NOTSET"] | int | None
    ...
    *acts* on it is the method's own decision (e.g. the AutoGluon wrappers gate task-specific
    ...
    def fit_custom(
        ...
        *,
        ...
        inside the timed fit (``fit`` is called with ``data_owned=True``, so the wrappers append the label
        ...
        with cpu_tracker, gpu_tracker:
            ...
            with Timer() as timer_fit:
                # The frames are this call's own (see above), so the fit may edit them in place.
                self.fit(X_fit, y_fit, data_owned=True)
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, X_val=None, y_val=None, *, data_owned: bool = False):
        ...
        Args:
            X: Training features.
            y: Training target.
            X_val: Optional validation features, transformed like ``X``.
            y_val: Optional validation target.
            data_owned: Whether the frames belong to this call and may be edited in place (a wrapper
                appending the label column, a system editing the frame). ``fit_custom`` passes True
                because it hands over a lazy-loaded frame, a shuffled copy, the feature generator's
                output or a copy it took before the fit timer; a direct caller keeps the default False
                and its frames are copied before any edit.
        """
        ...
        return self._fit(X=X, y=y, X_val=X_val, y_val=y_val, data_owned=data_owned)

    def _fit(self, X: pd.DataFrame, y: pd.Series, X_val=None, y_val=None, *, data_owned: bool = False):
        """Fit the underlying model on already-preprocessed data. Must be implemented.

        ``data_owned`` says whether ``X`` (and ``X_val``) may be edited in place; see ``fit``.
        """
