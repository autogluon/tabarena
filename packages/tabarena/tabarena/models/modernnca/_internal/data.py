# from https://github.com/LAMDA-Tabular/TALENT/blob/cb6cb0cc9d69ac75c467e8dae8ca5ac3d3beb2f2/TALENT/model/lib/data.py#L1
# and https://github.com/LAMDA-Tabular/TALENT/blob/cb6cb0cc9d69ac75c467e8dae8ca5ac3d3beb2f2/TALENT/model/lib/TData.py#L1
...
class TData(Dataset):
    ...
    def get_categories(self):
        return (
            ...
            else [len(set(self.X_cat[:, i].cpu().tolist())) for i in range(self.X_cat.shape[1])]
        )
...
def get_dataset(dataset_name, dataset_path):
    ...
    N_trainval = (
        None if N is None else {key: N[key] for key in ["train", "val"]} if "train" in N and "val" in N else None
    )
    N_test = None if N is None else {key: N[key] for key in ["test"]} if "test" in N else None

    C_trainval = (
        None if C is None else {key: C[key] for key in ["train", "val"]} if "train" in C and "val" in C else None
    )
...
def data_nan_process(
    N_data, C_data, num_nan_policy, cat_nan_policy, num_new_value=None, imputer=None, cat_new_value=None
):
    ...
    if C_data is None:
        ...
        cat_nan_masks = {
            k: np.isnan(v) if np.issubdtype(v.dtype, np.number) else np.isin(v, ["nan", "NaN", "", None])
            for k, v in C.items()
        }
    ...
    return (N, C, num_new_value, imputer, cat_new_value)
...
def data_enc_process(N_data, C_data, cat_policy, y_train=None, ord_encoder=None, mode_values=None, cat_encoder=None):
    ...
    if C_data is not None:
        ...
        result = (C_data, None) if N_data is None else ({x: np.hstack((N_data[x], C_data[x])) for x in N_data}, None)
        return result[0], result[1], ord_encoder, mode_values, cat_encoder
...
def data_loader_process(is_regression, X, Y, y_info, device, batch_size, is_train, is_float=False):
    ...
    if is_regression:
        Y = {k: v.float() for k, v in Y.items()} if is_float else {k: v.double() for k, v in Y.items()}
    else:
        Y = {k: v.long() for k, v in Y.items()}

    loss_fn = F.mse_loss if is_regression else F.cross_entropy
...
def get_categories(
    X_cat: dict[str, torch.Tensor] | None,
) -> list[int] | None:
    ...
    """
    return None if X_cat is None else [len(set(X_cat["train"][:, i].tolist())) for i in range(X_cat["train"].shape[1])]
