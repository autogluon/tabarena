...
- **`benchmark-model`** — Use whenever a maintainer wants to *run* an already-integrated model on the benchmark cluster (e.g. "benchmark TabM", "run Nori on the cluster", "make a setup/eval script for DenseLight"). It scaffolds a single `tmp_scripts/run_<model>.py` with `setup` and `eval` subcommands that share one `benchmark_name` + `PathSetup` (so they can't drift), auto-filling GPU/CPU, eval `subsets`, the pip-extra install reminder, and foundation-model prefetch by introspecting the model's registry `info.py` + `supported_problem_types()`. Sits in the lifecycle between `add-model` (integrate) and `upload-method` (publish).
