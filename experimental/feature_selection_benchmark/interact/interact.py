...
class INTERACTFeatureSelector(AbstractFeatureSelector):
    ...
    def _fit_feature_scoring(self, *, X: pd.DataFrame, y: pd.Series, time_limit: int | None = None) -> dict[str, float]:
        ...
        if (time_limit is not None) and (elapsed_time >= time_limit):
            ...
            feature_scores = dict(zip(X.columns, np.array(slist, dtype=int)))
    ...
    def _c_contribution(self, X: pd.DataFrame, y: pd.Series, feature_set: list[int], f_idx: int, time_limit, start_time) -> float:
        """
