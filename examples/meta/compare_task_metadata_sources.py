...
2. **New** — ``TaskMetadataCollection.from_preset("TabArena-v0.1")`` loads the
   native task metadata, which we then convert to the legacy DataFrame via
   :meth:`~tabarena.benchmark.task.metadata.TaskMetadataCollection.to_legacy_df`.
...
def load_via_collection() -> pd.DataFrame:
    """Method 2: native collection -> legacy DataFrame (metadata-only, no downloads)."""
    return TaskMetadataCollection.from_preset("TabArena-v0.1").to_legacy_df()
...
def compare_columns(old: pd.DataFrame, new: pd.DataFrame) -> None:
    ...
    print(f"collection + to_legacy    : {new.shape[0]:>4} rows x {new.shape[1]:>3} cols")
    ...
    print(f"collection-only columns ({len(new_cols - old_cols)}): {sorted(new_cols - old_cols)}")
...
def compare_datasets(old: pd.DataFrame, new: pd.DataFrame) -> list[str]:
    ...
    print(f"collection datasets : {len(new_ds)}")
    print(f"shared datasets : {len(shared)}")
    if old_ds - new_ds:
        print(f"legacy-only datasets: {sorted(old_ds - new_ds)}")
    if new_ds - old_ds:
        print(f"collection-only datasets: {sorted(new_ds - old_ds)}")
    return shared
...
def compare_values(old: pd.DataFrame, new: pd.DataFrame, shared_datasets: list[str]) -> None:
    ...
    merged = o.join(n, lsuffix="_legacy", rsuffix="_collection")

    for col in cols:
        left, right = merged[f"{col}_legacy"], merged[f"{col}_collection"]
        ...
        if n_eq != len(merged):
            diff = merged.loc[~equal, [f"{col}_legacy", f"{col}_collection"]]
