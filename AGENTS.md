...
The *inner* validation protocol is how a bagged model is validated inside one outer split: the bagging fold and repeat counts, an optional tiny-data regime, whether the inner splits follow the task's group / time structure, and whether AutoGluon may shrink the fold count to the minority-class count. It is one object, `ValidationProtocol` (`packages/tabarena/src/tabarena/benchmark/validation_protocol.py`), and two constants: `TABARENA_V0PT1_VALIDATION_PROTOCOL` (8 folds x 1 set, plain stratified splits, key `8x1`) and `BEYONDARENA_VALIDATION_PROTOCOL` (8x1 with 5x5 at or below 500 training group instances, task-specific splits, class-adaptive folds, key `8x1+tiny5x5<=500+task-specific+adapt-classes`). Identity is the field-derived `key()`, never the name label.

The arena context owns it. `TabArenaContext` and `BeyondArenaContext` declare theirs as `OFFICIAL_VALIDATION_PROTOCOL`; `build_jobs` / `run_jobs` stamp it onto every bagged, holdout and full-predictor experiment that carries none, refuse a bagged experiment that carries another protocol (the error names both protocols and the paired bundle), refuse a `method_kwargs["validation_metadata"]` split-structure override, and check that the task collection still carries the official split structure and splits. The official bundles (`TabArenaV0pt1ExperimentBundle`, `BeyondArenaExperimentBundle`) set no protocol, so context plus bundle is the one correct pipeline for a submission. The same assertion runs at every later stage: the tabflow head node refuses a cached result fit under another protocol, the `JobBatch` ships the context's `ValidationExpectation` (`validation_protocol.json`) and the compute node re-checks the experiment against it, `Experiment.task_cache_scope` refuses a bagged or holdout fit without a protocol, and a results cache carries a `validation_protocol.json` side record so a rerun under another protocol never reuses it.

Only bagged models and systems are official flavours (`OFFICIAL_VALIDATION_FLAVOURS`); outer, holdout and full-predictor fits run outside the protocol by construction, need no flag, and are recorded as such. To run bagged experiments under another protocol, build the context with `official_validation_protocol=False` and choose the protocol at one of three levels: the context (`validation_protocol=`), the bundle or `ModelJob.experiment` (per model group), or a single experiment / `generate_bag_experiments(validation_protocol=...)` (per config); `examples/advanced/run_custom_validation_protocol.py` shows all three. A bare `AbstractArenaContext` enforces exactly the protocol it is given.

Every result records what it ran under (`results["validation_protocol"]`: flavour, protocol, composite key, resolved and fitted counts, the data-dependent clamps, the split structure, the bag's child count and `use_child_oof`). Downstream, the per-result frame carries `vp_*` columns, `MethodMetadata.validation_protocol` holds the key (`"8x1"`, the BeyondArena key, `"system"`, `"<flavour>:<key>"` for non-bagged fits, `"mixed"`, or `None` for results that predate the record), the rosters declare it for every hosted method, processing fails when a new run's `info.py` leaves it undeclared, `run_process_method.py --expect-validation-protocol` asserts the arena's protocol for a submission, and the contexts warn when a registered method ran a custom protocol. KNN is the one declared outlier: its bag is a single child with leave-one-out predictions (`use_child_oof`), recorded as `child_oof=True` and listed as such by the inspector, never as a deviation.

"Protocol" is used for five different things in this repo; keep them apart: the outer split scheme of a task collection (folds x repeats of the benchmark), the task-aware inner splitting (group / time structure), the bagging protocol above (the `ValidationProtocol`), the tuning protocol (search space, time limits, the default / tuned / tuned + ensembled variants), and BeyondArena's `core` and `core2k` subsets.
...
Model libraries with a checkpoint cache of their own are not managed here: TabPFN keeps its `.ckpt` files under `~/.cache/tabpfn` (not `HF_HOME`), and moving that is done with tabpfn's own `TABPFN_MODEL_CACHE_DIR` variable, set before the first tabpfn import (the SLURM job environment carries it to the compute nodes; the TabPFN wrappers' `prefetch_weights` fill whatever directory tabpfn resolves). The opt-in weight staging of the SLURM runner copies those files to node-local scratch for one job and points the variable there for that job only.
...
1. **Author + process** — `scripts/run_process_method.py` (logic in `tabarena.tools.process_local_raw_data`):
   - Inspect the raw dir for a suggested metadata snippet: `python scripts/run_process_method.py <run_dir>/data` (recursive; prints the inferred fields + a `MethodMetadata.config/.baseline/.portfolio(...)` snippet). Paste it into `packages/tabarena/src/tabarena/models/<model>/info.py` (or `systems/<system>/info.py` for a system) and fill in `suite` (required, must differ from `method`) + the manual fields.
   - Process: append `--method-metadata tabarena.models.<model>.info:<x>_method_metadata --process`. **Processing requires an explicit `MethodMetadata`**, verified against the raw data first (method_type / compute / ag_key / `config_default` / can_hpo / is_bag / `validation_protocol` must align, and `method != suite`); a run whose results record a validation protocol must declare it in `info.py` (raw artifacts written before the record existed pass), and `--expect-validation-protocol tabarena|beyondarena` additionally asserts the arena's official protocol for a submission; a `method`-name mismatch only warns and `model_key` is not checked, other mismatches error unless `--ignore-metadata-mismatch`. A declared `config_default` is compared post-rename and is the usual failure: the TabArena-v0.1 bundle appends the preprocessing pipeline name to every config id, so a run's first config is `<Method>_c1_default_BAG_L1` (HPO or not), while `info.py` files authored before the run often carry the older `<Method>_c1_BAG_L1`; set it to the inferred value. A single-config method (`can_hpo=False`) may leave `config_default` unset: it is not checked, the lone config is its default, and processing records the resolved name in the cached `metadata.yaml`. HPO methods should declare it. Caches `metadata.yaml` + `processed/` + `results/` (plus raw + HPO trajectories, on by default) under the TabArena cache.
...
- **Warm-up and the timing audit**: every timed fit is preceded by an untimed environment warm-up (`AbstractExecModel.warmup_fn`, layered per model class in `tabarena/models/warmup.py`: an optional `warmup` classmethod, torch and the CUDA context, the class's `warmup_modules`, a dummy fit on synthetic data). A wrapper declares its lazy imports in `warmup_modules` and its cheapness knobs in `cheap_hyperparameters`. A foundation model declares `shared_weights = SharedWeights(loader=..., key=..., disabled_by=...)` (from `autogluon.core.models.abstract`), naming the library call that builds its network and the inputs that decide which network it is; AutoGluon's `AbstractTorchModel` then builds that network once per process (the warm-up's dummy fit is the first build), pickles the fitted model without it and takes it back on load, and `_fit` does not change. When a library stands in the way of the warm-up or the sharing (it loads inside a constructor, loads lazily at the first predict, or flips a global torch flag when imported), the wrapper carries a small workaround whose docstring or comment opens with `Developer fix`, names the upstream gap, what the library should offer instead and the upstream issue or PR when one exists (`exaone_tabular/_estimators.py`, the eager predictor build in `nori/model.py`, the pinned TF32 flag in `iltm/model.py`); grep for the header to find them, and remove each once the library ships the fix, as TabDPT's constructor replica went when layer6ai-labs/TabDPT-inference#79 merged (`tabdpt/model.py` declares the upstream `_load_model`). The add-model skill (Step 3g) has the library contract and the pattern. The fairness contract (data-independent work only) is in the `warmup.py` docstring. Check a model with `python -P -m tabarena.tools.audit_warmup --model <Method>` from the repo root (the timed fit and predict should import no new package), a finished run with `--results <output dir>`; `tests/tabarena/models/test_warmup_coverage.py` (marker `models`) asserts it per registered model. Both see the main process only: parallel Ray fold workers start cold. A warm-up that raised or left steps failed aborts the run before the timed fit (`ExperimentRunner(require_warmup=True)`, the default; the SLURM runner's `--require_warmup false` or `TabArenaBenchmarkPlan(require_warmup=False)` bypasses it), so a cold timed fit is never recorded silently.
- **Result metadata**: on top of the timing and memory keys, a `results.pkl` carries `experiment_metadata["warmup_report"]`, `["timing_audit"]`, `["cpu_thread_info"]` and `["time_warmup_s"]`, and the method metadata `shared_weights`, `persisted_models`, `prepared_for_inference` and `per_child_test_source` (`"timed_children_pass"` when the timed predict ran AutoGluon's `predict_proba_children` once and derived the bag's output from the arrays, `"timed_prediction"` for a single-child bag on the predictor's own path, `"child_forward_pass"` when the children predicted a second time). Results written before a key existed lack it, so consumers read these with `.get` (the `_collect_memory_usage` docstring in `exec_models/base.py` lists the baseline shifts of the memory keys).
...
- PR description: a two-to-four sentence summary first, then a `<details><summary>Details</summary>` block (collapsed by default) with the longer description. Both are written for the humans reviewing the PR; keep the whole body short.
- Use `.github/pull_request_template.md`. For a model or system submission fill in the "Model or system submission" section; otherwise delete it. Keep the closing contribution line.
- Issues use the forms in `.github/ISSUE_TEMPLATE/` (submission questions; leaderboard or dataset questions that touch this code). The contribution process (evaluate on TabArena-Lite first, maintainers verify and re-run, leaderboard updated after merge) is in the README section "Contributing a Model or System" and applies when an agent drafts a submission PR too.
