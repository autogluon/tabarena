...
from tabarena.models._weights import make_key
from tabarena.utils import ray_utils


def _registry():
    """The live weights registry module.

    Looked up by name on every call because ``test_lazy_imports.py`` purges ``tabarena.models.*``
    from ``sys.modules``; ``warmup.py`` imports the registry by name too, so both sides must agree.
    """
    return importlib.import_module("tabarena.models._weights")
...
class _SharedWeightsModel:
    ...
    @classmethod
    def shared_weights_key(cls, *, problem_type, hyperparameters, device):
        cls.key_calls.append({"problem_type": problem_type, "hyperparameters": hyperparameters, "device": device})
        return make_key("fake", "ckpt.pt", problem_type, device)

    @classmethod
    def _load_shared_weights(cls, key):
        cls.loads.append(key)
        return {"network": key.variant}


class _UnshareableModel(_SharedWeightsModel):
    @classmethod
    def shared_weights_key(cls, **kwargs):
        return None
...
class _CheapMeanModel(_MeanModel):
    ...
    warmup_dummy_fit_hyperparameters = {"n_estimators": 1}
...
def test_declared_warmup_classmethod_gets_full_context():
    ...
    assert report.steps == [
        "warmup:_DeclaredWarmupModel",
        "weights:_DeclaredWarmupModel:skipped",
        "dummy_fit:_DeclaredWarmupModel:skipped",
    ]
...
def test_unknown_model_is_a_noop(monkeypatch):
    ...
    assert report.steps == ["weights:_PlainModel:skipped", "dummy_fit:_PlainModel:skipped"]
...
def test_shared_weights_step_primes_the_fit_key(clean_registry):
    _SharedWeightsModel.key_calls.clear()
    _SharedWeightsModel.loads.clear()
    hps = {"n": 1, "ag_args_fit": {"num_gpus": 0}, "ag_args_ensemble": {"x": 1}}
    report = wu.warmup_model_cls(_SharedWeightsModel, problem_type="binary", num_gpus=0, hyperparameters=hps)
    assert _SharedWeightsModel.key_calls == [{"problem_type": "binary", "hyperparameters": {"n": 1}, "device": "cpu"}]
    key = make_key("fake", "ckpt.pt", "binary", "cpu")
    assert _SharedWeightsModel.loads == [key]
    assert clean_registry.contains(key) and clean_registry.loaded_by(key) == "warmup"
    assert "weights:_SharedWeightsModel" in report.steps
    assert len(report.weights_preloaded) == 1
    assert report.weights_preloaded[0]["variant"] == "binary" and report.weights_preloaded[0]["loaded_by"] == "warmup"
    # Priming again is idempotent: the loader does not run twice.
    wu.warmup_model_cls(_SharedWeightsModel, problem_type="binary", num_gpus=0, hyperparameters=hps)
    assert len(_SharedWeightsModel.loads) == 1


def test_shared_weights_step_config_gpu_wins_over_job_value(clean_registry):
    _SharedWeightsModel.key_calls.clear()
    wu.warmup_model_cls(
        _SharedWeightsModel, problem_type="regression", num_gpus=0, hyperparameters={"ag_args_fit": {"num_gpus": 1}}
    )
    assert _SharedWeightsModel.key_calls[-1]["device"] == "cuda"


def test_shared_weights_step_skips_without_hooks_key_or_problem_type(clean_registry):
    report = wu.warmup_model_cls(_UnshareableModel, problem_type="binary", num_gpus=0)
    assert "weights:_UnshareableModel:skipped" in report.steps and report.weights_preloaded == []
    report = wu.warmup_model_cls(_SharedWeightsModel, problem_type=None, num_gpus=0)
    assert "weights:_SharedWeightsModel:skipped" in report.steps
    assert clean_registry.report()["entries"] == []


def test_shared_weights_loader_failure_is_recorded(clean_registry, monkeypatch):
    def boom(cls, key):
        raise OSError("checkpoint missing")

    monkeypatch.setattr(_SharedWeightsModel, "_load_shared_weights", classmethod(boom))
    report = wu.warmup_model_cls(_SharedWeightsModel, problem_type="binary", num_gpus=0)
    assert report.failed_steps == ["weights:_SharedWeightsModel:failed:OSError"]
    assert report.weights_preloaded == []
