...
class _RecordingExecModel(AbstractExecModel):
    ...
    def __init__(self, *args, fit_hook=None, **kwargs):
        ...
        self.fit_X = None
        self.fit_y = None
        self.owned_during_fit = None
        self._fit_hook = fit_hook

    def _fit(self, X, y, data_owned=False, **kwargs):
        self.fit_X = X
        self.fit_y = y
        self.owned_during_fit = data_owned
...
def test_fit_custom_copies_the_caller_frame_before_the_fit():
    ...
    assert model.owned_during_fit is True  # the fit worked on an owned frame ...
...
def test_direct_fit_does_not_own_the_caller_frame():
    X, y, _ = _frames()
    model = _model()
    model.fit(X, y)
    assert model.owned_during_fit is False
    model.fit(X.copy(), y, data_owned=True)
    assert model.owned_during_fit is True
